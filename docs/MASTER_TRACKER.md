...
- [x] Commit gate: complete normal tests, coverage, deterministic mutation sampling, deterministic fuzz sampling and security verification
- [x] Certification gate: full mutation, extended fuzz, race/concurrency, stress, fault injection, resource leaks, full UI exploration, dependency/security/history scanning, sandbox behavior and reproducibility
...
- [ ] Cairo
- [x] HTML
- [x] CSS, SCSS, Sass, Less and Stylus source families
- [x] Markdown and MDX source families
- [x] Web template and single-file-component source families
- [ ] GLSL
- [ ] HLSL
- [ ] WGSL
...
### Web development ecosystem coverage
- [x] JavaScript web runtimes: Node.js, Deno and Bun detection
- [x] JavaScript package managers: npm, pnpm, Yarn and Bun package-manager detection with standard script routing
- [x] UI frameworks: React, Preact, Vue, Angular, Svelte, SolidJS, Qwik, Lit, Alpine.js and HTMX inventory
- [x] Full-stack/meta/static frameworks: Next.js, Nuxt, SvelteKit, Astro, Remix, React Router, Gatsby, Eleventy, Docusaurus and VitePress inventory
- [x] Web server/API frameworks: Express, Fastify, NestJS, Koa, Hono, h3, Socket.IO and ws inventory
- [x] Web build and monorepo tooling: Vite, webpack, Rollup, esbuild, Parcel, Rspack, Turborepo and Nx inventory
- [x] Web test stacks: Node test runner, Vitest, Jest, Playwright, Cypress, Mocha and AVA inventory
- [x] Styling ecosystems: Tailwind CSS, PostCSS, Sass, Less, styled-components and Emotion inventory
- [x] Web deployment/platform markers: Vercel, Netlify, Cloudflare Workers/Pages and Firebase inventory
- [x] Web ecosystem security scan rejects high-confidence remote-shell lifecycle scripts
- [x] Web UI/API applicability routes detected surfaces to explicit fail-closed verification harnesses
...
HTML, CSS/stylesheets, Markdown and web template/SFC formats are now first-class through the native web-family adapter crate. HTML receives structural tag/comment/quote validation and interactive-control inventory; CSS covers CSS, SCSS, Sass, Less and Stylus source files with delimiter/comment/string validation and selector/custom-property inventory; Markdown performs fenced-code structural validation and heading inventory; template/SFC coverage includes Vue, Svelte, Astro, MDX, EJS, Handlebars, Pug, Nunjucks, Liquid and Mustache-style source formats with package-script-aware build/type/lint/test routing. Web source checks include deterministic formatting, reference/dependency inventory, high-confidence source security checks, unfinished-marker scanning, impact-targeted testing, and explicit fail-closed UI/API/advanced harness routing. A repository-level web ecosystem specialist inventories Node.js, Deno and Bun; npm, pnpm, Yarn and Bun package management; major UI, meta/full-stack, server/API, build, monorepo, test, styling and deployment ecosystems; it also blocks high-confidence remote-shell package lifecycle patterns and emits evidence-backed ecosystem inventory. Dedicated Web Development Family CI proves real HTML, CSS/SCSS, Markdown and Vue-style SFC repositories plus a broad ecosystem matrix through the public CLI while keeping the existing JavaScript/TypeScript language adapters responsible for JS/TS syntax, type, package-script and dependency behavior. The legacy aggregated `html-css` fallback is suppressed in the CLI when these native adapters are present so native and fallback verification cannot double-detect the same source. Assembly is now first-class through the native Assembly adapter, covering GNU-style `.s`/`.S` and NASM-style `.asm` sources. The adapter inventories labels and exported symbols, assembles build/type/lint outputs outside the repository, assembles/links/executes native test programs, inventories include directives, blocks unfinished or credential-like source markers, and fails closed to explicit harnesses for advanced checks. Dedicated Assembly CI proves independent real GNU and NASM repositories through the public CLI, and the same exact implementation head passes Script Family CI, JVM Family CI, and the complete VerificationForge CI including real CommitGate and CertificationGate proofs. Bash, PowerShell and PHP are now first-class through the native script-family adapter crate. Bash uses native `bash -n` syntax checking plus pinned ShellCheck linting; PowerShell uses the native PowerShell AST parser plus pinned PSScriptAnalyzer; PHP uses native `php -l` parsing with Composer-aware dependency and security paths. All three provide independent detection and symbol inventory, native test execution, authenticity checks, checkpoint applicability, and fail-closed advanced harnesses. Dedicated Script Family CI passes real Bash, PowerShell and PHP fixtures through the public CLI, while the exact same implementation head passes JVM regression and the complete VerificationForge CI including real CommitGate and CertificationGate proofs. Java, Kotlin and Scala are now first-class through the native JVM-family adapter crate. The three adapters detect and inventory their source independently, use compiler-backed parse/build/type/lint checks, run source-only executable tests, integrate Maven/Gradle/sbt dependency and test paths where present, block placeholder/fake-authorization patterns, and fail closed to explicit per-language harnesses for advanced checks that cannot be proven generically. Dedicated JVM CI pins Java 21, Kotlin 2.4.10 and Scala CLI 1.16.0 and passes real zero-dependency Java, Kotlin and Scala fixtures through the public CLI; the same exact implementation head also passes the complete existing VerificationForge CI including the real CommitGate and CertificationGate proofs. Rust, Python and Go have native first-class adapters; the Go adapter performs native module detection, symbol inventory, build/type/vet/test/dependency/coverage/race verification, optional govulncheck integration, Go-specific advanced harness fallback, and placeholder/fake-authorization blocking. JavaScript and TypeScript are now first-class through the native JavaScript-family adapter crate, with independent source detection and symbol inventory, Node/TypeScript parse and type checks, package-script-aware build/lint/test execution, deterministic format fallback, placeholder/fake-authorization blocking, dependency/security integration where the package manager supports it, and explicit fail-closed harnesses for advanced checks that cannot be proven generically. C and C++ are first-class through the native C-family adapter crate with compiler syntax/type/lint checks, translation-unit builds to temporary objects outside the repository, compiled executable tests, dependency extraction, symbol inventory and authenticity checks. The same family crate provides native C# detection, project/solution builds and warning-as-error linting through dotnet, native test-project execution, dependency/vulnerability inspection and C# authenticity checks. CI verifies real zero-dependency JavaScript, TypeScript, C, C++ and C# fixtures through those native adapters, asserts one native detection per fixture, requires evidence-backed PASS results, and retains the existing real Rust/Python/Go, fake-implementation rejection and unknown-language rejection proofs. Mixed-language verification, specification-to-obligation generation, graph primitives and evidence-gap queries, content-addressed snapshot/impact planning, resource-aware scheduling, risk-tier policy, evidence-backed PASS semantics, and the controlled development firewall are implemented in the workspace. The integrated verification graph validates requirement-to-symbol links and binds evidence to the exact linked implementation symbol and check; a requirement is only proven when every linked implementation symbol has reproducible passing evidence, while bare PASS values and failures remain non-proving records. Controlled-operation provenance telemetry records agent identity, requirement scope, files, symbols, exact commands, outcomes, evidence, fix attempts, regression tests and timing without exposing a mutable bypass around the firewall. The staged artifact provenance API enforces agent -> requirement -> patch -> verification -> commit -> build -> artifact ordering and gives each final chain a deterministic content address with artifact, requirement and commit traceability. The recoverable run supervisor persists task state and checkpoints atomically and distinguishes heartbeat loss (hang), continued-heartbeat/no-progress stalls, and dependency-cycle deadlocks. A separate adversarial-review contract can create additional verification obligations but intentionally has no PASS, acceptance or certification capability. The default repository-wide authenticity specialist runs at every verification level and blocks high-confidence unfinished/fake implementation patterns independently of language adapters; CI proves that a Go project with a successful build and passing tests is still rejected when its authorization decision is hardcoded, while VerificationForge's own embedded test-fixture source is not misclassified. The agent-facing development runtime owns the tracked controlled session, requires a non-empty requirement before build work, exposes no mutable inner-session escape, and routes repository mutation, development commands, tests, commits and certification requests through the firewall and telemetry; tests prove direct .git file mutation, evidence-free commits and commits with active engine blockers are rejected. The strict content-addressed PatchGate composes repository snapshots, changed-path and dependency-cone impact analysis, parse/format/build/type/lint checks, secret and authenticity specialists, and impact-targeted tests; Unsupported required phases and bare PASS values fail closed. Rust supplies native parse/format and Cargo-package-targeted test execution with safe full-workspace fallback when impact cannot be mapped. CI exercises the Patch gate with real temporary Rust packages and proves both that a clean mapped patch is accepted and that a patch whose build and targeted tests pass is still rejected when it introduces a hardcoded credential. The compositional CheckpointGate requires PatchGate acceptance first, then affected integration and property verification, repository security, dependency verification, and explicit UI/API applicability outcomes. Native Rust maps affected paths to Cargo packages, requires real integration and property coverage for affected Rust packages, permits non-applicable skips only through explicit adapter outcomes, and fails closed when an affected UI/API surface lacks a checkpoint harness. CI proves a real Rust checkpoint with integration/property coverage is accepted and separately proves that ordinary PatchGate and integration tests can pass while CheckpointGate still rejects the change when property verification disappears. The strict CommitGate requires CheckpointGate acceptance first, then full normal tests and coverage for every detected language, repository-wide content-addressed deterministic mutation and fuzz sampling through exact argv harnesses that must consume the seed, selection count and iteration count, repository security specialist evidence, and a final content-address stability proof. Missing or nondeterministic sample harnesses fail closed, mutation and fuzz seeds change when repository content changes, and CI proves the complete gate against a real temporary Rust crate with a locked dependency state and real mutation/fuzz executables. The strict CertificationGate composes CommitGate and uses content-addressed deterministic work plans for full mutation, extended fuzz, concurrency, stress, fault injection, resource-leak verification, UI exploration when graph-applicable, sandbox verification and reproducibility; it also requires dependency checks, repository security evidence, Git-history secret scanning and a final repository-stability proof. Its public hardened acceptance layer fails closed unless each applicable declared workload is fully accounted for: full mutation must prove a nonzero discovered set with discovered=executed and zero survivors, iteration-based phases must meet their declared work counts, resource checks must prove the declared sample count with zero leaks, and sandbox checks must prove the declared case count with zero escapes. CI proves this hardened gate end to end against a real temporary Rust repository with locked dependency state, real Git history, mutation killing, deterministic fuzzing, concurrency, stress, injected failures, descriptor-leak checks, sandbox path attacks and deterministic reproducibility; a second real case proves that a credential committed and later deleted still blocks certification through history scanning. Generic fallback profiles remain available for many other ecosystems and require real per-language harness evidence rather than fabricating PASS. Items remain unchecked when only part of the compound requirement exists; complete per-operation resource observability, shared/distributed cache, remote isolation backends, comprehensive fake-implementation semantics, broader adversarial suites and VerificationForge self-certification are still incomplete.
